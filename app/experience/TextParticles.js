...
const TextParticles = (text) => {
	...
	// const SIZE = 32;
	const letters = _.filter([...text], l => l.match(/[a-zA-Z0-9]/i));
	...
	const fragmentSimulationShader = `
		...
		void main() {
			...
			vec3 wind = normalize(noise) * WIND_STRENGTH;
	...
	const vertexShader = `
		...
		void main(){
			...
			vec3 particlePosition = data * 1000.0;
			vUv = uv;
	...
	const update = (correction) => {
		...
		simulationMaterial.uniforms.uCorrection.value = 1;
